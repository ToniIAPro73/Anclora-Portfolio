import React, { useRef, useEffect } from "react";
import { motion, useInView } from "framer-motion";

interface WaterfallChartProps {
  data: { label: string; value: number; isTotal?: boolean; color?: string }[];
  width?: number;
  height?: number;
  className?: string;
}

export const WaterfallChart: React.FC<WaterfallChartProps> = ({
  data,
  width = 600,
  height = 400,
  className,
}) => {
  const ref = useRef<SVGGElement>(null);
  const isInView = useInView(ref, { once: true });

  const padding = { top: 20, right: 20, bottom: 40, left: 60 };
  const chartWidth = width - padding.left - padding.right;
  const chartHeight = height - padding.top - padding.bottom;

  // Calculate cumulative values
  const cumulativeData = data.reduce(
    (acc, item, index) => {
      const prevValue = index === 0 ? 0 : acc[index - 1].cumulative;
      const cumulative = prevValue + item.value;
      acc.push({
        ...item,
        cumulative,
        prevValue,
        isPositive: item.value >= 0,
      });
      return acc;
    },
    [] as Array<{
      label: string;
      value: number;
      isTotal?: boolean;
      color?: string;
      cumulative: number;
      prevValue: number;
      isPositive: boolean;
    }>,
  );

  const allValues = cumulativeData.flatMap((d) => [d.prevValue, d.cumulative]);
  const minValue = Math.min(...allValues);
  const maxValue = Math.max(...allValues);
  const valueRange = maxValue - minValue || 1;

  const getCategoryX = (index: number) => {
    const categoryWidth = chartWidth / data.length;
    return padding.left + index * categoryWidth + categoryWidth / 2;
  };

  const getValueY = (value: number) => {
    const normalized = (value - minValue) / valueRange;
    return padding.top + chartHeight - normalized * chartHeight;
  };

  const getConnectorPath = (index: number) => {
    const current = cumulativeData[index];
    const next = cumulativeData[index + 1];
    if (!next) return "";

    const startX = getCategoryX(index) + 10;
    const startY = getValueY(current.cumulative);
    const endX = getCategoryX(index + 1) - 10;
    const endY = getValueY(next.prevValue);

    return `M ${startX} ${startY} L ${endX} ${endY}`;
  };

  return (
    <svg
      width="100%"
      height={height}
      viewBox={`0 0 ${width} ${height}`}
      className={`overflow-visible ${className}`}
    >
      <defs>
        <linearGradient
          id="waterfallGradient"
          x1="0%"
          y1="0%"
          x2="0%"
          y2="100%"
        >
          <stop offset="0%" stopColor="#C5A059" stopOpacity="0.8" />
          <stop offset="100%" stopColor="#D4B77A" stopOpacity="0.3" />
        </linearGradient>
      </defs>
      <g ref={ref}>
        {/* Y-axis grid lines */}
        {[0, 0.25, 0.5, 0.75, 1].map((ratio, i) => (
          <line
            key={i}
            x1={padding.left}
            y1={padding.top + chartHeight * ratio}
            x2={width - padding.right}
            y2={padding.top + chartHeight * ratio}
            stroke="#E8E4E0"
            strokeWidth="1"
            strokeDasharray={ratio === 0.5 ? "4 4" : "none"}
          />
        ))}

        {/* Y-axis labels */}
        {[maxValue, (maxValue + minValue) / 2, minValue].map((value, i) => (
          <text
            key={i}
            x={padding.left - 10}
            y={getValueY(value) + 4}
            textAnchor="end"
            fontSize="10"
            fill="#94A3B8"
            fontFamily="var(--font-montserrat)"
          >
            €{Math.round(value).toLocaleString()}
          </text>
        ))}

        {/* X-axis labels */}
        {data.map((item, index) => (
          <text
            key={index}
            x={getCategoryX(index)}
            y={height - 10}
            textAnchor="middle"
            fontSize="10"
            fill="#64748B"
            fontFamily="var(--font-montserrat)"
          >
            {item.label}
          </text>
        ))}

        {/* Axis title */}
        <text
          x={width / 2}
          y={height - 2}
          textAnchor="middle"
          fontSize="10"
          fill="#94A3B8"
          fontFamily="var(--font-montserrat)"
        >
          Categories
        </text>

        {/* Waterfall bars */}
        {cumulativeData.map((item, index) => {
          const color = item.color || (item.isPositive ? "#C5A059" : "#1E293B");
          const categoryX = getCategoryX(index);
          const barWidth = (chartWidth / data.length) * 0.6;

          const startY = getValueY(item.prevValue);
          const endY = getValueY(item.cumulative);
          const barHeight = Math.abs(endY - startY);
          const barY = Math.min(startY, endY);

          return (
            <g key={index}>
              {/* Connector line */}
              {index < cumulativeData.length - 1 && (
                <motion.path
                  d={getConnectorPath(index)}
                  fill="none"
                  stroke="#E8E4E0"
                  strokeWidth="2"
                  initial={{ opacity: 0, pathLength: 0 }}
                  animate={
                    isInView
                      ? { opacity: 1, pathLength: 1 }
                      : { opacity: 0, pathLength: 0 }
                  }
                  transition={{ delay: index * 0.2 + 0.3, duration: 0.5 }}
                />
              )}

              {/* Bar */}
              <motion.rect
                x={categoryX - barWidth / 2}
                y={barY}
                width={barWidth}
                height={barHeight}
                fill={color}
                stroke="#FAF9F6"
                strokeWidth="2"
                rx={item.isTotal ? 4 : 0}
                initial={{ opacity: 0, height: 0, y: barY + barHeight / 2 }}
                animate={
                  isInView
                    ? { opacity: 1, height: barHeight, y: barY }
                    : { opacity: 0, height: 0, y: barY + barHeight / 2 }
                }
                transition={{ delay: index * 0.2 + 0.5, duration: 0.6 }}
              />

              {/* Value label */}
              <motion.text
                x={categoryX}
                y={barY - 8}
                textAnchor="middle"
                fontSize="10"
                fontWeight="600"
                fill="#1E293B"
                fontFamily="var(--font-montserrat)"
                initial={{ opacity: 0 }}
                animate={isInView ? { opacity: 1 } : { opacity: 0 }}
                transition={{ delay: index * 0.2 + 0.8, duration: 0.4 }}
              >
                {item.value >= 0 ? "+" : ""}
                {item.value.toLocaleString()}
              </motion.text>

              {/* Cumulative label */}
              <motion.text
                x={categoryX}
                y={barY + barHeight + 16}
                textAnchor="middle"
                fontSize="9"
                fontWeight="400"
                fill="#64748B"
                fontFamily="var(--font-montserrat)"
                initial={{ opacity: 0 }}
                animate={isInView ? { opacity: 1 } : { opacity: 0 }}
                transition={{ delay: index * 0.2 + 1.0, duration: 0.4 }}
              >
                Total: {item.cumulative.toLocaleString()}
              </motion.text>
            </g>
          );
        })}
      </g>
    </svg>
  );
};
